...
func (heap *heap) Pop() Key {
	...
	key := heap.keys[1]            // retrieve key from top, order depends on Key.CompareTo implementation
	exchange(heap.keys, 1, heap.N) // exchange with last item
	heap.N--
	heap.keys[heap.N+1] = nil // avoid loitering
	sink(heap.keys, 1, heap.N)
	return key
}
